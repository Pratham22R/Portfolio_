            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 pt-4">
              {stats.map((stat, index) => (
                <div
                  key={stat.label}
                  className={cn(
                    "glass-card p-3 md:p-4 text-center transition-all duration-700 transform hover:shadow-lg hover:shadow-blue-500/10 group",
                    isInView ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                  )}
                  style={{ transitionDelay: `${index * 100}ms` }}
                >
                  <div className="text-xl md:text-2xl lg:text-3xl font-bold group-hover:gradient-text transition-colors duration-300">{stat.value}</div>
                  <div className="text-xs text-muted-foreground group-hover:text-muted-foreground/80 transition-colors duration-300">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
